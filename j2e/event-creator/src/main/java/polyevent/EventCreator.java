...
public class EventCreator implements IEventCreator {
    ...
    @Override
    public polyevent.entities.Event registerEvent(String name, int participantNumber, Calendar date, polyevent.entities.Coordinator coordinator) throws InvalidRequestParametersException, RoomNotAvailableException, InvalidRoomException, DatabaseSavingException {
        ...
        polyevent.entities.Event event = new polyevent.entities.Event(coordinator, date.getTime(), cal.getTime(), participantNumber, name);
    ...
    @Override
    public boolean cancelEvent(polyevent.entities.Event event) {
        return memory.deleteEvent(event);
    }

    /**
     * Checks that all the parameters received for an {@link polyevent.entities.Event} creation are valid
     * and returns true if they are, or false if they're not
     * @param name the name of the {@link polyevent.entities.Event}, as a {@link String}
     * @param participantNumber the estimated number of people for the
     *                          {@link polyevent.entities.Event} as a {@link Integer}
     * @param date the date at which the {@link polyevent.entities.Event} is scheduled,
     *             as a {@link Calendar}
     * @param coordinator the {@link polyevent.entities.Coordinator} that created the {@link Event}
